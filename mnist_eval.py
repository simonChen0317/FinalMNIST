...
def evaluate(mnist):
    with tf.Graph().as_default() as g:
        ...
        #这里直接通过调用封装好的函数来计算前向传播的结果。因为测试时不关注正则化损失的值，所以这里用于计算正则化损失的函数设置为None。
        ...
        #通过变量重命名的方式来加载模型，这样在前向传播的过程中就不需要调用求滑动平均的函数来获取平均值了。这样就可以完全共用mnist_inferende.py中定义的前向传播过程。
        ...
        #每隔EVAL_INTERVAL_SECS秒调用一次计算正确率的过程以检验训练过程正确率的变化。
        while True:
